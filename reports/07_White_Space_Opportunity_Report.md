...
- **Digital Twin + Embedded AI / Edge Inference**: EIS-RV style (your existing) for batteries/electrochem. Extend to AFPM: Real-time state estimation (temp, flux, eccentricity), anomaly detection, efficiency optimization via local LLM or tinyML on ESP32/STM32 (your hardware strength). Closes loop from operation data back to design refinement (NeuroFlux training data flywheel).
